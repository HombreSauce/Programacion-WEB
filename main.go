...
func main() {
	...
	filas, err := queries.EliminarUsuario(ctx, 1)
	if err != nil {
		log.Fatalf("No se pudo borrar el usuario: %v", err)
	}
	if filas == 0 {
		log.Fatalf("No se encontró el usuario con ID 1")
	} else {
		fmt.Printf("El usuario ID 1 se borró satisfactoriamente")
	}
	...
	//ELIMINAMOS MEDICO
	filas, err = queries.EliminarMedico(ctx, 2)
	if err != nil {
		log.Fatalf("No se pudo borrar el medico: %v", err)
	}
	if filas == 0 {
		log.Fatalf("No se encontró el medico con ID 2")
	}
