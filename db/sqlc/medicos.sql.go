...
const eliminarMedico = `-- name: EliminarMedico :execrows
DELETE FROM medicos
WHERE id_medico = $1
`

func (q *Queries) EliminarMedico(ctx context.Context, idMedico int32) (int64, error) {
	result, err := q.db.ExecContext(ctx, eliminarMedico, idMedico)
	if err != nil {
		return 0, err
	}
